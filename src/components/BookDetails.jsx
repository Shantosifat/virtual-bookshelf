...
const [editing, setEditing] = useState(false);
  ...
  try {
    ...
    setBook(res.data); // update UI with new review list
    setReviewText("");
    setEditing(false);
  } catch (error) {
...
const handleDeleteReview = async () => {
  ...
  try {
    ...
    setBook(res.data); // update book with updated reviews
    setReviewText("");
    setEditing(false);
  } catch (error) {
  ...
  <div className="max-w-5xl mx-auto py-12 px-4  lg:px-6">
    {/* Book Details */}
    <div className="flex flex-col md:flex-row bg-slate-100 shadow-lg rounded-2xl p-6 md:p-8 transition hover:shadow-2xl">
      ...
      <div className="flex-1 space-y-4">
        ...
        <div className="space-y-1 text-gray-700">
          ...
          <p><span className="font-semibold text-gray-900">Status:</span> {status}</p>
        </div>
        ...
        <button
          ...
          className="mt-4 inline-flex items-center px-6 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition"
        >
          {upvote ? "Voted" : "Vote"}
        </button>
      </div>
...
<div className="mt-10 bg-slate-100 rounded-2xl p-4 text-stone-950">
  ...
  {user ? (
    ...
    {/* <div className="mt-16">
      <div className="bg-slate-100 rounded-2xl shadow-lg p-6 md:p-10">
